...
dat <- DBI::dbGetQuery(conn = con_udal, statement = "
...
on c.stp_code = h.icb_code
  ...
  where month > DATEADD(mm,-18,GETDATE())                    
")
...
df <- df |>
    mutate(
      ics_name = integrated_care_board_name,
      ics_name = case_when(
    ...
    ics_name = tolower(integrated_care_board_name),
    region = tolower(region_name_2),
    period = month,
...
df <- df |>
  mutate(
    icb = str_remove(integrated_care_board_name, "NHS "),
...
df <- df |>
  mutate(
    ...
    child = if_else(grepl("(CYP)", service, fixed = TRUE), 1, 0)
  ) # adds a flag for childrens service in case need to split)
...
total <- df |>
  ...
  mutate(service = if_else(child == 0, "Total waits adults", "Total waits children"))
...
df <- df |>
  mutate(ics_name = case_when(
    ics_name == "Cornwall and The Isles Of Scilly" ~ "Cornwall",
...
df_tot_icb <- df |>
  ...
  summarise(tot = sum(metric_value, na.rm = TRUE)) |>
  mutate(
...
df_refs_reg <- df |>
  filter(metric_id %in% c("RES001ci", "RES001cii", "RES001ciii")) |>
  group_by(
    region,
    month,
    service,
  ...
  summarise(tot = sum(metric_value)) |>
  mutate(
    tot = replace_na(tot, 0),
    tot_rate = round(tot / pop_reg, 1),
    area = region
...
# calculate 18, 52 / 104 weeks by provider
df_waits_prov <- df |>
  filter(metric_id %in% c("RES001cvi", "RES001cvii", "RES001cviii", "RES001c"),
         region == reg) |>
  mutate(metric_value = replace_na(metric_value, 0), ) |>
  pivot_wider(names_from = metric_id, values_from = metric_value) |>
  ungroup() |>
  select(org_name,
         service_short,
         month,
         ...
         RES001cviii,
         RES001c) |>
  summarise(RES001cvi = sum(RES001cvi, na.rm = T),
            RES001cvii = sum(RES001cvii, na.rm = T),
            RES001cviii = sum(RES001cviii, na.rm = T),
            RES001c = sum(RES001c, na.rm = T),
            .by = c(org_name,
                    service_short,
                    child,
                    month))|>
  mutate(
    perc_18 = round(sum(((RES001cvi + RES001cvii + RES001cviii) / RES001c) * 100, na.rm = TRUE), 1),
    perc_52 = round(sum(((RES001cvii + RES001cviii) / RES001c) * 100, na.rm = TRUE), 1),
    perc_104 = round(sum(((RES001cviii) / RES001c) * 100, na.rm = TRUE), 1),
    .by = c(org_name,
                    service_short,
                    child,
                    month)) |>
  mutate(region = reg,
         area = org_name,
    area = str_remove(area, " NHS"),
    area = str_remove(area, " FOUNDATION"),
    area = str_remove(area, " HOSPITALS"),
    area = str_remove(area, " TRUST"))

# calculate latest percentage of over 52 / 104 weeks by ics
# just snapshot of latest month
df_long_ics <- df |>
  filter(metric_id %in% c("RES001cvii", "RES001cviii", "RES001c")) |>
  mutate(metric_value = replace_na(metric_value, 0), ) |>
  group_by(
    ics_name,
    service,
  ...
  pivot_wider(names_from = metric_id, values_from = metric_value) |>
  mutate(
    perc_long = round(sum(((RES001cvii + RES001cviii) / RES001c) * 100, na.rm = TRUE), 1),
    num_long = sum(RES001cvii + RES001cviii),
    perc_long = replace_na(perc_long, 0),
    perc_v_long = round(sum(((RES001cviii) / RES001c) * 100, na.rm = TRUE), 1),
...
plot_serv <- function(data, serv, title, val = 'tot_rate', wait = '') {
  df_service <- data |>
    filter(
      service_short == serv,
      region == reg
  ...
  plot_title <- if (title == "tot") {
    ...
    paste0("SPC chart for ", serv, " - Percentage of patients waiting over ", wait)
  } else {
...
# test plots
#serv <- '(A) Therapy interventions'
#plot_serv(df_refs_icb,'(A) Therapy interventions', "ref")
#plot_serv(df_tot_reg, '(A) Therapy interventions', "tot")
...
dat_summary <- function(serv) {
  ...
  dat_t <- dat |>
    ...
    mutate(
      ...
      perc_long = paste0(perc_long,'% (',RES001cvii+RES001cviii,')'),
      perc_v_long = paste0(perc_v_long,'% (',RES001cviii,')')
    ) |>
...
create_table <- function(serv) {
  ...
  highlight <- min(which(grepl(tolower(reg), dat_table$area)))
  ...
  dat_table |>
    gt() |>
    fmt_number(columns = tot, use_seps = TRUE, decimals = 0) |>
    fmt_number(columns = tot_rate, use_seps = TRUE, decimals = 1) |>
    # create the rate graph (col 4)
    gt_plt_bar_pct(
      column = rate_graph, scaled = FALSE,
      fill = "#005EB8", background = "lightblue"
    ) |>
    fmt_icon(columns = c(rate_trend, data, ref_trend,data_ref_flag)) |>
    ...
    # add a heading title 
    tab_header(
      title = paste0('Community waiting list as at ', format (ceiling_date(max(df_refs_reg$month), 'month') - days(1), '%d %B %y'), ' : ', serv ),
       subtitle = md("Standardised rates, trend, referrals, breakdown by long waits")
    ) |> 
    ...
    tab_footnote(
      footnote = "Data taken from COVID-19 daily discharge situation report (SitRep) "
    ) |>
...
# test tables
# create_table('Total waits adults')
...
reg_par <- reg_over |>
  filter(!service %in% c("Total waits patients", "Total waits adults", "Total waits children"))
...
p <- ggplot(reg_par, aes(x = service)) +
  geom_bar(
    ...
    stat = "identity"
  ) + # basic plot by service
  geom_path(
    aes(
      y = cumulative / scaleRight,
      group = 1
    ),
    colour = "red",
    linewidth = 0.9
  ...
  geom_errorbar(
    ...
    position = "dodge",
    width = 0.2
  ) + # add error bars for min and max my regions
  geom_point(
    aes(
      x = service,
      y = nat_rate
...
service_types <- df_tot_reg |>
  filter (month == max(df_tot_reg$month),
          region == reg,
          !service_short %in% c('Total waits patients', 'Total waits adults', 'Total waits children'),
          tot > 0) |>
  arrange(desc(tot))

list_order <- c('Total waits patients', 'Total waits adults', 'Total waits children',service_types$service_short)
...
order_list <- c('highest', 'second highest', 'third highest', 'middle', 'third lowest', 'second lowest', 'lowest')

commentary <- function (w_list) {
#pull the overall summary data
summary_data <- dat_summary(w_list) |>
  mutate(area = str_to_title(area))
...
line_one <- paste0('The **',reg,'** has a total of **', pn(total_waiting_region), ' ', word(w_list,-1), '** waiting, this equates to **',pn(rate_waiting_region),' per 100,000** population against the National total of **', pn(total_waiting_national), ' (',pn(rate_waiting_national),' per 100,000).**  This puts the **', reg,'** region as the *',order_list [region_rank], '* in the country of the seven regions.')
...
line_four <- paste0('For those waiting over 52 weeks, **',reg, '** is the *', order_list[region_rank_52], '* of the seven regions with **',region_detail_52,'** ', word(w_list,-1), ' waiting over 52 weeks.')
...
line_five <- paste0('Within the region, **', icb_high_52, '** has the highest proportion of 52 week waits with **', icb_perc_52, '** ',word(w_list,-1),' waiting over 52 weeks.')
...
line_six <- paste0('For those waiting over 104 weeks, **',reg, '** is the *', order_list[region_rank_104], '* of the seven regions with **',region_detail_104,'** ', word(w_list,-1), ' waiting over 104 weeks.')
...
line_seven <- paste0('Within the region, **', icb_high_104, '** has the highest proportion of longest waits with **', icb_perc_104, '** ',word(w_list,-1), ' waiting over 104 weeks.')
...
local_services <- reg_over |>
  ...
  filter(area == reg,
         ...
         child == if_else (w_list == 'Total waits adults',0, 1)) |>
...
### Combined Community Waiting List

* `r tot_com[1]`  

* `r tot_com[2]` 

### Adults Community Waiting Lists  
...
### Childrens Community Waiting Lists  
...
## Regional and ICB comparison (click to expand)
...
::: {.callout-tip collapse="true" appearance="default" icon="true"}
## Regional snapshot -(click to expand)
...
::: {.callout-tip collapse="true" appearance="default" icon="true"}
## Chart Interpretation (click to expand)

- The letter in the top right corner represents the overall trend, L - numbers getting lower (improvement), H - numbers getting higher (deterioration), C - common cause variation (no discernible trend)  
...
NOTE: This shows percentage of patients waiting over 18 weeks.
...
## Detailed charts of percentage of patients waiting over 18 weeks by provider and service
...
#| output: asis
 res <- purrr::map_chr(list_order, \(hw) {
                           knitr::knit_child(text = c(
                             ...
                             "#| fig-width: 16",
                             "#| fig-height: 8", 
                             "plot_serv(df_waits_prov, hw, 'perc', 'perc_18', '18 weeks') +
...
## Detailed charts of percentage of patients waiting over 52 weeks by provider and service
...
#| output: asis
 res <- purrr::map_chr(list_order, \(hw) {
                           knitr::knit_child(text = c(
                             ...
                             "#| fig-width: 16",
                             "#| fig-height: 8", 
                             "plot_serv(df_waits_prov, hw, 'perc', 'perc_52', '52 weeks') +
                             scale_y_continuous(labels = scales::percent_format( scale = 1))+
                             xlab (NULL)",
                             "```",
                             "",
...
#| output: asis
 res <- purrr::map_chr(list_order, \(hw) {
                           knitr::knit_child(text = c(
                             ...
                             "#| fig-width: 16",
                             "#| fig-height: 8", 
                             "plot_serv(df_waits_prov, hw, 'perc', 'perc_104', '104 weeks') +
                             scale_y_continuous(labels = scales::percent_format( scale = 1))+
                             xlab (NULL)",
                             "```",
                             "",
...
NOTE: This shows percentage of patients waiting over 104 weeks.
...
::: {.callout-caution collapse="true" appearance="default" icon="true"}
...
Trend information is calculated using Statistical Process Control rules, using NHSE Making Data Count methodology https://www.england.nhs.uk/publication/making-data-count  
...
The R code to produce this report is available at https://github.com/nhsengland/community-waiting-list.
:::

> Contact  
> [**Simon Wellesley-Miller**](mailto:Simon.Wellesley-Miller@nhs.net)\
> *Senior Analytical Manager*  
> Version 1.2 1 April 2025
